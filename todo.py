...
def read_file(path):
    ...
    todo_list = read_file_looper("#TODO","@todo(getDef='true')", todo_list, file_obj)
...
def read_file_looper(search,subsearch, lst, source):
    """
    ...
    (lst) and then look in the same line for the subsearch. If found it will call the find_string function with 'def ' as the search param.

    Returns lst

    :param search: primary search term to find
    :type search: string
    :param subsearch: secondary search term to find
    ...
    for num, line in enumerate(source, 1):
        if search in line:
            ...
            if subsearch in line:
               find_string('def ',lst, enumerate(source, num)) 

    return lst

def find_string(search, lst, source):
    """
    find_string loops through the source and tries to find lines matching the search param. If found it will add the line to the provided list
    and then return true. Note, this method will exit after finding the first instance of the search come accross

    returns boolean depending if search was found

    :param search: term to look for
    :type search: string
    :param lst: list to add the matching lines to
    :param source: source loop. Unlike read_file_looper this function expects an already enumerated loop
    """ 
    for num, line in source:
        if search in line:
            lst.append(_remove_whitespace(line))
            return True # only get one result
